...
def callback_left_odom(data):
    ...
    lr_pose[2] = euler_from_quaternion(qtr)
...
def callback_right_odom(data):
    ...
    rr_pose[2] = euler_from_quaternion(qtr)
...
rospy.Subscriber('/left_odom', Odometry,
                 callback_left_odom)  # topic name fixed
rospy.Subscriber('/right_odom', Odometry,
                 callback_right_odom)  # topic name fixed
...
while not rospy.is_shutdown():  # replace with balancing reached?
    ...
    v_lin, v_ang = velocity_convert(r_pose[2], v[0], v[1])
